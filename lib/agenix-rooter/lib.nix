...
let
  mixHash =
    ...
    (
      ...
      else if builtins.isPath el then
        (builtins.hashFile "sha512" el)
      else
        (builtins.hashString "sha512" el)
    );
  short = str: builtins.substring 0 32 str;
  hashForSecret =
    hostConfig: secret:
    short (mixHash [
      hostConfig.age.rooter.hostPubkey
      secret.rooterEncrypted
    ]);
in
rec {
  generatedSecretFilename =
    hostConfig: secret:
    "${hashForSecret hostConfig secret}-${hostConfig.networking.hostName}-${secret.name}.age";
  generatedSecretStorePath =
    hostConfig: secret:
    let
      ...
      generatedForHostStorePath =
        lib.throwIfNot (lib.pathExists hostConfig.age.rooter.generatedForHostDir)
          "${toString hostConfig.age.rooter.generatedForHostDir} does not exist. ${fixitCmd}"
          "${hostConfig.age.rooter.generatedForHostDir}";
  ...
  relativePath =
    ...
    lib.throwIfNot (lib.hasPrefix fromStr toStr)
