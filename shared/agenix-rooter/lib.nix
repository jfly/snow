...
let
  ...
  hashForSecret = hostConfig: secret: short (mixHash [ hostConfig.age.rooter.hostPubkey secret.rooterFile ]);
in
rec {
  ...
  generatedSecretStorePath = hostConfig: secret: "${hostConfig.age.rooter.generatedForHostDir}" + "/${generatedSecretFilename hostConfig secret}";
  ...
  relativePath = { from, to }:
    ...
    assert lib.assertMsg (lib.hasPrefix fromStr toStr) "'from' must be an ancestor of 'to'. Feel free to add support for more relative relationships if you need it.";
